...
let FlowField = function () {
    ...
    function createFlowBuilder(uComp, vComp, sc) {
        let uData = uComp.data, vData = vComp.data;
        return {
            header: uComp.header,
            //recipe: recipeFor("wind-" + uComp.header.surface1Value),
            data: function (i) {
                return [uData[i], vData[i], sc.data[i]];
            },
    ...
    function createBuilder(data) {
        ...
        return createFlowBuilder(uComp, vComp, scalar);
    }
        ...
        for (let j = 0; j < nj; j++) {
            ...
            for (let i = 0; i < ni; i++, p++) {
                ...
                dem.push({
                    ...
                    geometry: {
                        type: "Point",
                        coordinates: [X0 + i * deltaX, Y0 - j * deltaY]
                    },
                    properties: {
                        strength: Math.sqrt(Math.pow(row[i][0], 2) + Math.pow(row[i][1], 2)) / 10
                    }
                })
            }
            if (isContinuous) {
